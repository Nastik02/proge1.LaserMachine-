...
class StageViewSignals(QObject):
    mouseStageClicked = pyqtSignal(QPointF)
...
class QZoomStageView(QWidget):
    def __init__(self):
        ...
        self.__zoom = 1
        self.__oldMousePosition = QPointF()
        self.setMouseTracking(True)
        self.__isInDragMode = False
        self.canvas = QImage()
        self.init_image(self.size())
        self.__markerPoint = QPointF()
        self.__oldMarkerPoint = QPointF()
        self.__markerState = MarkerState.OFF
        self.__cameraPosition = QPointF()
    ...
    def showEvent(self, e) -> None:
        self.__cameraPosition = QPointF(10,
                                        40)  # QPointFF(self.frameSize().width() * 0.5, self.frameSize().height() * 0.5)
        self.set_zoom(10)
    ...
    def setCurrentPosition(self, position: QPointF):
        self.__markerPoint = position
        self.points.append(position)
        self.update()

    def setOldPosition(self, oldPosition: QPointF):
        self.__oldMarkerPoint = oldPosition
        # self.points.append(oldPosition)
        self.update()

    def setMarkerState(self):
        self.__markerState = MarkerState((self.__markerState + 1) % 3)
        self.update()
    ...
    def moveToClick(self, e: QPointF):
        matrix = QTransform()
        ...
        self.signals.mouseStageClicked.emit(QPointF(wx, -wy))
    ...
    def paintEvent(self, e: QPaintEvent):
        ...
        marker_size = max(1, 10 / (self.__zoom * 20))
        ...
        if self.__markerState != MarkerState.HIDE:
            if self.__markerState == MarkerState.ON:
                markerColor: QColor = Qt.GlobalColor.green
            else:
            ...
            painter.drawEllipse(QPointF(self.__markerPoint.x(), self.__markerPoint.y()), marker_size, marker_size)
        painter.setBrush(QBrush(Qt.GlobalColor.yellow))
        # painter.drawEllipse(int(self.__oldMarkerPoint.x()), int(self.__oldMarkerPoint.y()), marker_size, marker_size)
        painter.drawEllipse(QPointF(self.__oldMarkerPoint.x(), self.__oldMarkerPoint.y()), marker_size, marker_size)
        painter.translate(0.5 * marker_size, 0.5 * marker_size)
        painter.restore()
        ...
        painter.drawStaticText(QPointF(5, 50), QStaticText(f"State: {str(self.__markerState.name)}"))
        painter.drawStaticText(QPointF(5, 35), QStaticText(
            f"Old marker position: x = {self.__oldMarkerPoint.x()}, y = {self.__oldMarkerPoint.y()}"))